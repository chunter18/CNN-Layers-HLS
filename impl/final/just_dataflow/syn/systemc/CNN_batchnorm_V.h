...
#ifndef _CNN_batchnorm_V_H_
#define _CNN_batchnorm_V_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "CNN_batchnorm_V_memcore.h"

SC_MODULE(CNN_batchnorm_V) {
...
protected:
    ...
    CNN_batchnorm_V_memcore* CNN_batchnorm_V_memcore_U;
...
public:
    ~CNN_batchnorm_V();
    SC_CTOR(CNN_batchnorm_V) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        CNN_batchnorm_V_memcore_U = new CNN_batchnorm_V_memcore("CNN_batchnorm_V_memcore_U");
        CNN_batchnorm_V_memcore_U->clk      ( clk );
        CNN_batchnorm_V_memcore_U->reset    ( reset );
        CNN_batchnorm_V_memcore_U->ce0      ( i_ce0 );
        CNN_batchnorm_V_memcore_U->we0      ( i_we0 );
        CNN_batchnorm_V_memcore_U->address0 ( memcore_iaddr );
        CNN_batchnorm_V_memcore_U->d0       ( i_d0 );
        CNN_batchnorm_V_memcore_U->q0       ( i_q0 );

        CNN_batchnorm_V_memcore_U->ce1      ( t_ce0 );
        CNN_batchnorm_V_memcore_U->we1      ( t_we0 );
        CNN_batchnorm_V_memcore_U->address1 ( memcore_taddr );
        CNN_batchnorm_V_memcore_U->d1       ( t_d0 );
        CNN_batchnorm_V_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "CNN_batchnorm_V.trace");
            // ports
