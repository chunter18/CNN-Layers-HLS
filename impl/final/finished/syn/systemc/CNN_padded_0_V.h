...
#ifndef _CNN_padded_0_V_H_
#define _CNN_padded_0_V_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "CNN_padded_0_V_memcore.h"

SC_MODULE(CNN_padded_0_V) {
...
protected:
    ...
    // buffer instances
    CNN_padded_0_V_memcore* buffer[BufferCount];
...
public:
    ~CNN_padded_0_V();
    SC_CTOR(CNN_padded_0_V) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "CNN_padded_0_V_memcore_U_";
            name += szTmp;

            buffer[i] = new CNN_padded_0_V_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "CNN_padded_0_V.trace");
            // ports
