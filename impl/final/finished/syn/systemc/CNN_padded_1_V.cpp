...
void CNN_padded_1_V::proc_i_full_n() {
    i_full_n.write(full_n.read());
}

void CNN_padded_1_V::proc_t_empty_n() {
    t_empty_n.write(empty_n.read());
}

void CNN_padded_1_V::proc_i_q0() {
    i_q0.write(buf_q0[iptr.read()].read());
}

void CNN_padded_1_V::proc_t_q0() {
    t_q0.write(buf_q0[tptr.read()].read());
}

void CNN_padded_1_V::proc_i_q1() {
    i_q1.write(buf_q1[iptr.read()].read());
}

void CNN_padded_1_V::proc_t_q1() {
    t_q1.write(buf_q1[tptr.read()].read());
}

void CNN_padded_1_V::proc_buffer_signals() {
    for (unsigned i = 0; i < BufferCount; i++) {
        if (iptr.read() == i) {
...
void CNN_padded_1_V::proc_count() {
    if (reset.read() == SC_LOGIC_1) {
        count.write(0);
...
void CNN_padded_1_V::proc_full_n() {
    if (reset.read() == SC_LOGIC_1) {
...
void CNN_padded_1_V::proc_empty_n() {
    if (reset.read() == SC_LOGIC_1) {
