...
#ifndef _CNN_resampled_L2_hbi_H_
#define _CNN_resampled_L2_hbi_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "CNN_resampled_L2_hbi_memcore.h"

SC_MODULE(CNN_resampled_L2_hbi) {
...
protected:
    ...
    // buffer instances
    CNN_resampled_L2_hbi_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(CNN_resampled_L2_hbi) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "CNN_resampled_L2_hbi_memcore_U_";
            name += szTmp;

            buffer[i] = new CNN_resampled_L2_hbi_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "CNN_resampled_L2_hbi.trace");
            // ports
