...
#ifndef _CNN_resampled_L2_ibs_H_
#define _CNN_resampled_L2_ibs_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "CNN_resampled_L2_ibs_memcore.h"

SC_MODULE(CNN_resampled_L2_ibs) {
...
protected:
    ...
    // buffer instances
    CNN_resampled_L2_ibs_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(CNN_resampled_L2_ibs) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "CNN_resampled_L2_ibs_memcore_U_";
            name += szTmp;

            buffer[i] = new CNN_resampled_L2_ibs_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "CNN_resampled_L2_ibs.trace");
            // ports
